...
resource "google_monitoring_alert_policy" "database_connections" {
  ...
  conditions {
    ...
    condition_threshold {
      filter          = var.database_type == "cloudsql" ? "resource.type = \"cloudsql_database\" AND metric.type = \"cloudsql.googleapis.com/database/postgresql/num_backends\"" : "resource.type = \"alloydb_cluster\" AND metric.type = \"alloydb.googleapis.com/database/postgresql/num_backends\""
      duration        = "300s"
      comparison      = "COMPARISON_GT"
      threshold_value = 150
