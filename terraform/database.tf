...
resource "google_secret_manager_secret_version" "db_url" {
  ...
  secret_data = var.database_type == "cloudsql" ? 
    "postgresql://supabase:${random_password.db_password.result}@${google_sql_database_instance.postgres[0].private_ip_address}:5432/supabase" :
    "postgresql://postgres:${random_password.db_password.result}@${google_alloydb_instance.primary[0].ip_address}:5432/supabase"
}
...
output "database_connection_name" {
  value = var.database_type == "cloudsql" ? 
    ...
    "${google_alloydb_cluster.postgres[0].name}/instances/${google_alloydb_instance.primary[0].instance_id}"
}